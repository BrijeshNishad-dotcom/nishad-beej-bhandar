...
export default function DashboardOverview({ stats, recentEnquiries }: DashboardOverviewProps) {
  ...
  const handleDeleteEnquiry = async (id: number) => {
    if (!confirm('क्या आप इस पूछताछ को हटाना चाहते हैं?')) return;
    setIsUpdating(id);
    try {
      const res = await fetch(`/api/enquiries/${id}`, {
        method: 'DELETE',
  ...
  const statCards = [
    { name: 'कुल उत्पाद (Products)', value: stats.totalProducts, icon: ShoppingBag, color: 'text-blue-600 bg-blue-50 border-blue-100' },
    { name: 'श्रेणियां (Categories)', value: stats.totalCategories, icon: FolderOpen, color: 'text-amber-600 bg-amber-50 border-amber-100' },
    { name: 'प्राप्त पूछताछ (Enquiries)', value: stats.totalEnquiries, icon: MailOpen, color: 'text-green-600 bg-green-50 border-green-100' },
    { name: 'कुल विज़िटर्स (Visitors)', value: stats.totalVisitors, icon: Users, color: 'text-indigo-600 bg-indigo-50 border-indigo-100', subText: `आज: ${stats.visitorsToday} नए` },
  ];
    ...
    <div className="space-y-10">
      ...
      <div>
        <h1 className="font-display text-2xl sm:text-3xl font-extrabold text-agri-dark">
          एडमिन डैशबोर्ड (Dashboard Overview)
        </h1>
        <p className="font-sans text-sm text-gray-500 mt-1">
          Nishad Beej Bhandar स्टोर के आंकड़े एवं ग्राहक गतिविधियाँ
        </p>
      </div>
      ...
      <div className="bg-white border border-gray-150 rounded-3xl overflow-hidden shadow-sm">
        <div className="p-6 border-b border-gray-100 flex items-center justify-between">
          <h3 className="font-display font-bold text-base text-agri-dark">
            हालिया पूछताछ (Recent Farmer Enquiries)
          </h3>
                      ...
                      <div>
                        ...
                        <div className="flex flex-col text-xs text-gray-400 space-y-0.5 mt-1">
                          <span className="flex items-center">
                            <MapPin className="h-3 w-3 mr-1" /> ग्राम: {enq.village}
                          </span>
                          <span className="flex items-center">
